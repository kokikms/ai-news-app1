...
// 投稿時間を表示する関数
export function formatPublishDate(isoOrRfc: string | undefined): string | undefined {
  if (!isoOrRfc) return undefined;
...
// 相対時間を表示する関数（何時間前に投稿された）
export function getRelativeTimeText(isoOrRfc: string | undefined): string | undefined {
  if (!isoOrRfc) return undefined;
  const t = new Date(isoOrRfc).getTime();
  if (Number.isNaN(t)) return undefined;
  const diff = Date.now() - t;
